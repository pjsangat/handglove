...
(new WOW).init();var FACILITY_VOTES={init:function(){$(".datepicker").datepicker({format:"yyyy-mm-dd"}),FACILITY_VOTES.build.table(),FACILITY_VOTES.events.init()},events:{init:function(){FACILITY_VOTES.events.viewUnit(),FACILITY_VOTES.events.addUnit()},addUnit:function(){$("#submitUnit").click(function(){$.ajax({url:GLOBAL_BASE_URL+"facility/manage/votes/add",data:$("#unitAddForm").serialize(),type:"POST",dataType:"json",success:function(t){1==t.success&&(t='<div class="p-3 mb-2 bg-success text-white">'+t.message+"</div>",$("#formConfirmation").html(t),setTimeout(function(){$("#formConfirmation").html(""),$("#formConfirmation").hide()},5e3),$("#units_table").DataTable().ajax.reload(),$("#unitAddForm")[0].reset(),$("#addUnitModal").modal("hide"))}})})},removeUnit:function(){$("#units_table").on("click",".remove-unit",function(){})},viewUnit:function(){$("#units_table").on("click",".view-unit",function(){var t=$(this).data("id");$.ajax({url:GLOBAL_BASE_URL+"facility/manage/votes/get",data:{unitID:t},type:"POST",dataType:"json",success:function(t){if(1==t.success){$("#voting_start_date_label").text(t.unit.start_date),$("#voting_end_date_label").text(t.unit.end_date),$("#unitDescription").text(t.unit.description);for(var i="",a=0;a<t.unit.details.length;a++)i=(i=(i=(i=(i=(i+='<div class="row mb-3">')+'<div class="col-md-12">'+'<div class="clinician-details">')+('<div class="clinician-name">'+t.unit.details[a].clinician_details.name+"</div>")+'<div class="progress">')+('<div class="progress-bar bg-warning" role="progressbar" style="width: '+t.unit.details[a].vote_percentage+'%;" aria-valuenow="75" aria-valuemin="0" aria-valuemax="100"></div>'))+"</div>"+"</div>")+"</div>"+"</div>";$("#cliniciansVote").html(i),$("#unitModal").modal("show")}}})}),$("#updateUnit").click(function(){$.ajax({url:GLOBAL_BASE_URL+"facility/manage/units/update",data:$("#unitUpdateForm").serialize(),type:"POST",dataType:"json",success:function(t){1==t.success&&($("#units_table").DataTable().ajax.reload(),$("#unitUpdateForm")[0].reset(),$("#unitModal").modal("hide"))}})})}},build:{table:function(){$("#units_table").DataTable({paging:!0,lengthChange:!1,pageLength:10,ordering:!1,ajax:{url:GLOBAL_BASE_URL+"facility/manage/votes/list",dataSrc:"data",type:"POST"},columns:[{data:"name"},{data:"description"},{data:"action"}]})}}};$(document).ready(FACILITY_VOTES.init);