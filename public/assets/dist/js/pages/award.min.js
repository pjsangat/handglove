...
(new WOW).init();var AWARD={init:function(){$("#awardForm").validate({rules:{company_name:"required",company_address:"required",zip_code:"required",supervisor_name:"required",supervisor_name:"required",supervisor_contact_number:"required",supervisor_email:{required:!0,email:!0}}}),AWARD.events.submitAward()},events:{submitAward:function(){$("#submit-award").click(function(){$("#awardForm").valid()&&$.ajax({url:GLOBAL_BASE_URL+"employee-award/submit",type:"POST",data:{company_name:$("#company_name").val(),company_address:$("#company_address").val(),zip_code:$("#zip_code").val(),supervisor_name:$("#supervisor_name").val(),supervisor_email:$("#supervisor_email").val(),supervisor_contact_number:$("#supervisor_contact_number").val()},dataType:"json",success:function(e){1==e.success?($("#awardForm").remove(),$("#awardFormContainer").html(e.message)):$("#awardFormContainer").prepend(e.message)}})})}},build:{}};$(document).ready(AWARD.init);