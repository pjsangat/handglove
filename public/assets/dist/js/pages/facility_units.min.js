...
(new WOW).init();var FACILITY_UNITS={init:function(){FACILITY_UNITS.build.table(),FACILITY_UNITS.events.init()},events:{init:function(){FACILITY_UNITS.events.viewUnit(),FACILITY_UNITS.events.addUnit()},addUnit:function(){$("#submitUnit").click(function(){$.ajax({url:GLOBAL_BASE_URL+"facility/manage/units/add",data:$("#unitAddForm").serialize(),type:"POST",dataType:"json",success:function(t){1==t.success&&($("#units_table").DataTable().ajax.reload(),$("#unitAddForm")[0].reset(),$("#addUnitModal").modal("hide"))}})})},removeUnit:function(){$("#units_table").on("click",".remove-unit",function(){})},viewUnit:function(){$("#units_table").on("click",".view-unit",function(){var t=$(this).data("id");$.ajax({url:GLOBAL_BASE_URL+"facility/manage/units/get",data:{unitID:t},type:"POST",dataType:"json",success:function(t){1==t.success&&($("#unitID").val(t.unit.id),$("#unitName").val(t.unit.name),$("#unitCensus_1").val(t.unit.census_1),$("#unitCensus_2").val(t.unit.census_2),$("#unitDescription").val(t.unit.description),$("#unitModal").modal("show"))}})}),$("#updateUnit").click(function(){$.ajax({url:GLOBAL_BASE_URL+"facility/manage/units/update",data:$("#unitUpdateForm").serialize(),type:"POST",dataType:"json",success:function(t){1==t.success&&($("#units_table").DataTable().ajax.reload(),$("#unitUpdateForm")[0].reset(),$("#unitModal").modal("hide"))}})})}},build:{table:function(){$("#units_table").DataTable({paging:!0,lengthChange:!1,pageLength:10,ordering:!1,ajax:{url:GLOBAL_BASE_URL+"facility/manage/units/list",dataSrc:"data",type:"POST"},columns:[{data:"name"},{data:"census"},{data:"description"},{data:"action"}]})}}};$(document).ready(FACILITY_UNITS.init);