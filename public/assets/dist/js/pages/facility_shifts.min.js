...
(new WOW).init();var FACILITY_SHIFTS={init:function(){FACILITY_SHIFTS.events.init(),$(".datepicker").datepicker({format:"yyyy-mm-dd"})},events:{init:function(){FACILITY_SHIFTS.events.request(),FACILITY_SHIFTS.events.unitChanged(),FACILITY_SHIFTS.events.dateChanged(),FACILITY_SHIFTS.events.callOff(),FACILITY_SHIFTS.events.dnr(),FACILITY_SHIFTS.events.pcc(),FACILITY_SHIFTS.events.requestClinician(),FACILITY_SHIFTS.events.transferUnit()},request:function(){var a=$("#shiftUnit").val(),i=$("#shiftDate").val();$.ajax({url:GLOBAL_BASE_URL+"facility/manage/shifts/list",data:{unitID:a,date:i,clientTime:(new Date).getTime()},type:"POST",dataType:"json",success:function(a){if(void 0!==a.data&&0<a.data.length){for(var i="",t=0;t<a.data.length;t++)i+=FACILITY_SHIFTS.build.shift(a.data[t]);$("#facilityShifts").html(i)}else $("#facilityShifts").html("No shifts found..")}})},unitChanged:function(){$("#shiftUnit").change(function(){FACILITY_SHIFTS.events.request()})},dateChanged:function(){$("#shiftDate").change(function(){FACILITY_SHIFTS.events.request()})},pcc:function(){$("#facilityShifts").on("click",".pcc-action",function(){$("#pccModal").modal("show")})},callOff:function(){$("#facilityShifts").on("click",".co-action",function(){$("#calloffModal").modal("show")})},dnr:function(){$("#facilityShifts").on("click",".dnr-action",function(){$("#dnrModal").modal("show")})},requestClinician:function(){$("#facilityShifts").on("click",".request-clinician-table",function(){var a=$(this).data("id"),i=$(this).data("type");$("#clinicians_table").DataTable().destroy(),$("#clinicians_table").DataTable({paging:!0,lengthChange:!1,pageLength:10,searching:!1,ordering:!1,ajax:{url:GLOBAL_BASE_URL+"facility/manage/clinicians/list?type=request&shift="+a+"&shiftType="+i,dataSrc:"data",type:"POST"},columns:[{data:"details"},{data:"type"},{data:"level"},{data:"status"},{data:"action"}]}),$("#unitModal").modal("show")}),$("#clinicians_table").on("click",".request-clinician",function(){var a=$(this).data("id"),i=$(this).data("shift-id");$.ajax({url:GLOBAL_BASE_URL+"facility/manage/jobs/request",data:{clinicianID:a,shiftID:i},type:"POST",dataType:"json",success:function(a){1==a.success?(toastr.success(a.message,a.message_header),$("#clinicians_table").DataTable().ajax.reload()):toastr.error(a.message,a.message_header)}})})},transferUnit:function(){$("#facilityShifts").on("click",".transfer-unit-action",function(){var a=$(this).data("id"),i=$(this).data("shift-id");$.ajax({url:GLOBAL_BASE_URL+"facility/manage/shifts/transfer",data:{shift_clinician_id:a,new_shift_id:i},type:"POST",dataType:"json",success:function(a){1==a.success?(toastr.success(a.message,a.message_header),FACILITY_SHIFTS.events.request()):toastr.error(a.message,a.message_header)}})})}},build:{shift:function(a){s=(s="")+'<div id="facility1" class="shift"><label for="" style="font-size: 20px;">Shift # '+a.id+'</label><div class="shift-date-time"><div class="shift-icon">',"07:00 AM"==a.shift_start_time_formatted?s+='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="20" height="20" fill="currentColor" class="bi bi-sunrise text-warning"><path d="M7.646 1.146a.5.5 0 0 1 .708 0l1.5 1.5a.5.5 0 0 1-.708.708L8.5 2.707V4.5a.5.5 0 0 1-1 0V2.707l-.646.647a.5.5 0 1 1-.708-.708zM2.343 4.343a.5.5 0 0 1 .707 0l1.414 1.414a.5.5 0 0 1-.707.707L2.343 5.05a.5.5 0 0 1 0-.707m11.314 0a.5.5 0 0 1 0 .707l-1.414 1.414a.5.5 0 1 1-.707-.707l1.414-1.414a.5.5 0 0 1 .707 0M8 7a3 3 0 0 1 2.599 4.5H5.4A3 3 0 0 1 8 7m3.71 4.5a4 4 0 1 0-7.418 0H.499a.5.5 0 0 0 0 1h15a.5.5 0 0 0 0-1h-3.79zM0 10a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1h-2A.5.5 0 0 1 0 10m13 0a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1h-2a.5.5 0 0 1-.5-.5"></path></svg>':"03:00 PM"==a.shift_start_time_formatted?s+='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="20" height="20" fill="currentColor" class="bi bi-sun text-warning"><path d="M8 11a3 3 0 1 1 0-6 3 3 0 0 1 0 6m0 1a4 4 0 1 0 0-8 4 4 0 0 0 0 8M8 0a.5.5 0 0 1 .5.5v2a.5.5 0 0 1-1 0v-2A.5.5 0 0 1 8 0m0 13a.5.5 0 0 1 .5.5v2a.5.5 0 0 1-1 0v-2A.5.5 0 0 1 8 13m8-5a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1 0-1h2a.5.5 0 0 1 .5.5M3 8a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1 0-1h2A.5.5 0 0 1 3 8m10.657-5.657a.5.5 0 0 1 0 .707l-1.414 1.415a.5.5 0 1 1-.707-.708l1.414-1.414a.5.5 0 0 1 .707 0m-9.193 9.193a.5.5 0 0 1 0 .707L3.05 13.657a.5.5 0 0 1-.707-.707l1.414-1.414a.5.5 0 0 1 .707 0m9.193 2.121a.5.5 0 0 1-.707 0l-1.414-1.414a.5.5 0 0 1 .707-.707l1.414 1.414a.5.5 0 0 1 0 .707M4.464 4.465a.5.5 0 0 1-.707 0L2.343 3.05a.5.5 0 1 1 .707-.707l1.414 1.414a.5.5 0 0 1 0 .708"></path></svg>':s+='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="18" height="18" fill="currentColor" class="bi bi-moon text-primary"><path d="M6 .278a.77.77 0 0 1 .08.858 7.2 7.2 0 0 0-.878 3.46c0 4.021 3.278 7.277 7.318 7.277q.792-.001 1.533-.16a.79.79 0 0 1 .81.316.73.73 0 0 1-.031.893A8.35 8.35 0 0 1 8.344 16C3.734 16 0 12.286 0 7.71 0 4.266 2.114 1.312 5.124.06A.75.75 0 0 1 6 .278M4.858 1.311A7.27 7.27 0 0 0 1.025 7.71c0 4.02 3.279 7.276 7.319 7.276a7.32 7.32 0 0 0 5.205-2.162q-.506.063-1.029.063c-4.61 0-8.343-3.714-8.343-8.29 0-1.167.242-2.278.681-3.286"></path></svg>',s=(s=s+'</div><div class="shift-date">'+a.shift_start_time_formatted+" - "+a.shift_end_time_formatted+'</div></div><div class="shift-details">')+'<div class="shift-type">'+a.type_name+'</div><div class="shift-personnel">';for(var i=0;i<a.clinicians.length;i++){var t=a.clinicians[i].profile_pic_url;if(s=(s=(s+='<div class="personnel-details dropdown">')+'<div class="clinician-details" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="cursor: pointer;">'+('<img class="img-responsive" src="'+(t=""==t?GLOBAL_BASE_URL+"assets/img/blank-img.png":t)+'">'))+('<div class="clinician-name">'+a.clinicians[i].clinician_name+"</div>")+"</div>",a.alternative_units&&0<a.alternative_units.length&&!a.clinicians[i].is_clocked_in){for(var s=s+'<div class="dropdown-menu">'+'<h6 class="dropdown-header">Transfer to...</h6>',n=0;n<a.alternative_units.length;n++)s+='<a class="dropdown-item transfer-unit-action" href="javascript:;" data-id="'+a.clinicians[i].id+'" data-shift-id="'+a.alternative_units[n].shift_id+'">'+a.alternative_units[n].unit_name+"</a>";s+="</div>"}s=(s=(s=(s=(s=s+'<div class="clinician-actions">'+"<ul>")+('<li class=""><a href="javascript:;" class="pcc-action text-success" data-shift="'+a.id+'" data-clin="'+a.clinicians[i].id+'">'+(20==a.clinicians[i].pcc_status?'<i class="fa fa-unlock"></i>':'<i class="fa fa-lock"></i>')+" PCC</a></li>"))+('<li class=""><a href="javascript:;" class="co-action text-warning" data-shift="'+a.id+'" data-clin="'+a.clinicians[i].id+'"><i class="fa fa-phone-slash"></i> C/O</a></li>'))+('<li class=""><a href="javascript:;" class="dnr-action text-danger" data-shift="'+a.id+'" data-clin="'+a.clinicians[i].id+'"><i class="fa fa-eye-slash"></i> DNR</a></li>')+"</ul>")+"</div>"+"</div>"}return s=(s=s+a.action+"</div>")+"</div>"+"</div>"}}};$(document).ready(FACILITY_SHIFTS.init);