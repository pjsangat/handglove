...
(new WOW).init();var FACILITY_CLINICIANS={init:function(){FACILITY_CLINICIANS.build.table(),FACILITY_CLINICIANS.events.init()},events:{init:function(){FACILITY_CLINICIANS.events.viewClinician()},addClinician:function(){},removeClinician:function(){$("#clinicians_table").on("click",".remove-clinician",function(){alert("asddasdsa")})},viewClinician:function(){$("#clinicians_table").on("click",".view-clinician",function(){var i=$(this).data("id");$.ajax({url:GLOBAL_BASE_URL+"facility/manage/clinicians/get",data:{clinicianID:i},type:"POST",dataType:"json",success:function(i){1==i.success&&($("#clinicianName").text(i.clinician.name),$("#clinicianNameHeader").text(i.clinician.name+"'s Details"),$("#clinicianLevel").text(i.clinician.level),$("#clinicianAddress").text(i.clinician.address),$("#clinicianZip").text(i.clinician.zip_code),$("#clinicianEmail").text(i.clinician.email),$("#clinicianContact").text(i.clinician.contact_number),$("#clinicianType").text(i.clinician.type_name),$("#clinicianStatus").text(i.clinician.status_label),$("#clinicianModal").modal("show"))}})})}},build:{table:function(){$("#clinicians_table").DataTable({paging:!0,lengthChange:!1,pageLength:10,ordering:!1,ajax:{url:GLOBAL_BASE_URL+"facility/manage/clinicians/list",dataSrc:"data",type:"POST"},columns:[{data:"details"},{data:"type"},{data:"level"},{data:"status"},{data:"action"}]})}}};$(document).ready(FACILITY_CLINICIANS.init);