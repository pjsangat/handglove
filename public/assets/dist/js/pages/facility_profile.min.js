...
(new WOW).init();var PROFILE={init:function(){PROFILE.events.init()},events:{init:function(){PROFILE.events.vote()},vote:function(){$(".vote-clinician").click(function(){var t=$(this).data("voting_id"),i=$(this).data("clinician_id");$.ajax({url:GLOBAL_BASE_URL+"facility/vote",data:{votingID:t,clinicianID:i},type:"POST",dataType:"json",success:function(i){1==i.success&&(i='<div class="p-3 mb-2 bg-success text-white">'+i.message+"</div>",$("#voting"+t).html(i),setTimeout(function(){location.reload()},2e3))}})})}},build:{}};$(document).ready(PROFILE.init);